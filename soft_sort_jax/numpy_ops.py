...
from .third_party.isotonic import isotonic_l2_pav
...
def isotonic_l2(input_s, input_w=None):
  ...
  """
    ...
    solution = isotonic_l2_pav(input_s - input_w)
    return solution
...
class Isotonic(_Differentiable):
    ...
    def vjp(self, vector):
        start = 0
        return_value = jnp.zeros_like(self.solution_)
        for size in _partition(self.solution_):
            end = start + size
            if self.regularization == "l2":
                val = 1. / size
            ...
            return_value = return_value.at[start:end].set(val * np.sum(vector[start:end]))
            start = end
        return return_value
...
def _inv_permutation(permutation):
    ...
    inv_permutation = jnp.zeros(len(permutation), dtype=np.int)
